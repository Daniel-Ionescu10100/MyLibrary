...
public class BookMapper {
    ...
    {
        return new BookDTOBuilder().setTitle(book.getTitle()).setAuthor(book.getAuthor()).setQuantity(book.getQuantity()).setPrice(book.getPrice()).build();
    }

    public static Book convertBookDTOToBook(BookDTO bookDTO)
    {
        return new BookBuilder().setTitle(bookDTO.getTitle()).setAuthor(bookDTO.getAuthor()).setPublishedDate(LocalDate.of(2010,1,1)).setQuantity(bookDTO.getQuantity()).setPrice(bookDTO.getPrice()).build();
